...
class Unet(nn.Module):
    def __init__(self, args: "tupperware", in_c: int = 4):
        ...
        self.layer0_1x1 = convrelu(64, 64, 1, 0, normaliser=normaliser)

        self.layer1 = nn.Sequential(
            convrelu(64, 64, 3, 1, 2, normaliser=normaliser),
            convrelu(64, 64, 3, 1, 1, normaliser=normaliser),
        )  # size=(N, 64, x.H/4, x.W/4)
        self.layer1_1x1 = convrelu(64, 64, 1, 0, normaliser=normaliser)

        self.layer2 = nn.Sequential(
            convrelu(64, 128, 3, 1, 2, normaliser=normaliser),
            convrelu(128, 128, 3, 1, 1, normaliser=normaliser),
        )  # size=(N, 128, x.H/8, x.W/8)
        self.layer2_1x1 = convrelu(128, 128, 1, 0, normaliser=normaliser)

        self.layer3 = nn.Sequential(
            convrelu(128, 128, 3, 1, 2, normaliser=normaliser),
            convrelu(128, 256, 3, 1, 1, normaliser=normaliser),
        )  # size=(N, 256, x.H/16, x.W/16)
        self.layer3_1x1 = convrelu(256, 256, 1, 0, normaliser=normaliser)

        self.layer4 = nn.Sequential(
            convrelu(256, 512, 3, 1, 2, normaliser=normaliser),
            convrelu(512, 512, 3, 1, 1, normaliser=normaliser),
        )  # size=(N, 512, x.H/32, x.W/32)
        self.layer4_1x1 = convrelu(512, 256, 1, 0, 1, normaliser=normaliser)
        ...
        self.upsample_8_16 = UpsampleBLock(256, 2, normaliser=normaliser)
        self.upsample_16_32 = UpsampleBLock(256, 2, normaliser=normaliser)
        self.upsample_32_64 = UpsampleBLock(128, 2, normaliser=normaliser)
        self.upsample_64_128 = UpsampleBLock(64, 2, normaliser=normaliser)
        self.upsample_128_256 = UpsampleBLock(64, 2, normaliser=normaliser)

        self.conv_original_size0 = convrelu(
            in_c * args.pixelshuffle_ratio ** 2, 64, 3, 1, normaliser=normaliser
        )
        self.conv_original_size1 = convrelu(64, 64, 3, 1, normaliser=normaliser)
        self.conv_original_size2 = convrelu(64 + 64, 64, 3, 1, normaliser=normaliser)

        self.get_image_256 = nn.Sequential(
            nn.Conv2d(64, 3 * args.pixelshuffle_ratio ** 2, 1), nn.ReLU()
        )
    ...
    def forward(self, img):
        ...
        x_original = self.conv_original_size1(x_original)  # 64 channels

        layer0 = self.layer0(img)
        layer1 = self.layer1(layer0)
        layer2 = self.layer2(layer1)
        layer3 = self.layer3(layer2)
        layer4 = self.layer4(layer3)  # 512 channels

        layer4 = self.layer4_1x1(layer4)
        layer3 = self.layer3_1x1(layer3)

        x = self.upsample_8_16(layer4)  # size=(N, 512, x.H/16, x.W/16)
        x = torch.cat([x, layer3], dim=1)
        x = self.conv_up3(x)  # size=(N, 512, x.H/16, x.W/16)

        layer2 = self.layer2_1x1(layer2)
        x = self.upsample_16_32(x)  # size=(N, 512, x.H/8, x.W/8)
        x = torch.cat([x, layer2], dim=1)
        x = self.conv_up2(x)  # size=(N, 256, x.H/8, x.W/8)

        layer1 = self.layer1_1x1(layer1)
        x = self.upsample_32_64(x)  # size=(N, 256, x.H/4, x.W/4)
        x = torch.cat([x, layer1], dim=1)
        x = self.conv_up1(x)  # size=(N, 256, x.H/4, x.W/4)

        layer0 = self.layer0_1x1(layer0)
        x = self.upsample_64_128(x)  # size=(N, 256, x.H/2, x.W/2)
