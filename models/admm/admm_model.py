...
class ADMM_Net(nn.Module):
    def __init__(
        ...
        self.Hconj = self.H * torch.tensor(
            [1, -1], dtype=torch.float32, device=self.cuda_device
        )
        self.HtH = complex_abs(complex_multiplication(self.H, self.Hconj))

        self.LtL = torch.nn.Parameter(
            torch.tensor(
                make_laplacian(self), dtype=torch.float32, device=self.cuda_device
            ),
            requires_grad=False,
        )
    ...
    def initialize_learned_variables(self, learning_options):
        ...
        if "mus" in learning_options["learned_vars"]:  # Make mu parameters learnable
            ...
            self.mu2 = torch.nn.Parameter(
                torch.tensor(np.ones(self.iterations) * 1e-4, dtype=torch.float32)
            )
            self.mu3 = torch.nn.Parameter(
                torch.tensor(np.ones(self.iterations) * 1e-4, dtype=torch.float32)
            )
        else:  # Not learnable
            self.mu1 = (
                torch.ones(
                    self.iterations, dtype=torch.float32, device=self.cuda_device
                )
                * 1e-4
            )
            self.mu2 = (
    ...
    def forward(self, inputs):
        ...
        if self.l_admm_s == True:
            ...
            alpha2k = torch.zeros_like(
                Lsk_init, dtype=torch.float32, device=self.cuda_device
            )
        else:
