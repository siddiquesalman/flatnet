...
def admm(model, in_vars, alpha2k_1, alpha2k_2, CtC, Cty, mu_auto, n, y):
    ...
    Smult = 1 / (
        mu1 * model.HtH + mu2 * model.LtL + mu3
    )  # May need to expand dimensions
    Vmult = 1 / (CtC + mu1)

    ###############  update u = soft(Ψ*x + η/μ2,  tau/μ2) ###################################
    Lsk1, Lsk2 = L_tf(sk)  # X and Y Image gradients
    ukp_1, ukp_2 = soft_2d_gradient2_rgb(
        model, Lsk1 + alpha2k_1 / mu2, Lsk2 + alpha2k_2 / mu2, tau
    )
    ...
    wkp = torch.max(alpha3k / mu3 + sk, torch.tensor(0.0).to(mu3.device), out=None)
    ...
    skp = make_real(
        torch.ifft(complex_multiplication(make_complex(Smult), SKP_numerator), 2)
    )
    ...
    dual_resid_u.append(
        mu2 * torch.sqrt(torch.norm(Lsk1 - Lskp1) ** 2 + torch.norm(Lsk2 - Lskp2) ** 2)
    )
    primal_resid_u.append(torch.sqrt(torch.norm(r_su_1) ** 2 + torch.norm(r_su_2) ** 2))

    if model.autotune == True:
        mu2_up = param_update2(
            mu2,
            model.resid_tol,
            model.mu_inc,
            model.mu_dec,
    ...
    if model.printstats == True:
        ...
        admmstats = {
            "dual_res_s": dual_resid_s[-1].cpu().detach().numpy(),
        ...
        print(
            ...
            n,
            "s:",
            admmstats["dual_res_s"],
            admmstats["primal_res_s"],
            "u:",
            admmstats["dual_res_u"],
            admmstats["primal_res_u"],
            "w:",
            admmstats["dual_res_w"],
            admmstats["primal_res_w"],
            end="",
        )
    else:
        admmstats = []
