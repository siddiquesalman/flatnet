...
def pad_zeros_torch_CtC(model, x):
    f_inv = int(1 / model.factor)
    if f_inv != 1:
        offset_y = model.args.meas_crop_size_x % f_inv != 0
        offset_x = model.args.meas_crop_size_y % f_inv != 0
    else:
        offset_x = 0
        offset_y = 0

    crop_size_x = model.args.meas_crop_size_x
    crop_size_y = model.args.meas_crop_size_y

    psf_h = model.args.psf_height
    psf_w = model.args.psf_width

    pad_x_0 = int(model.factor * (psf_h - crop_size_x) // 2) + model.PAD_SIZE0
    pad_x_1 = (
        int(model.factor * (psf_h - crop_size_x) // 2) + model.PAD_SIZE0 + offset_x
    )
    pad_y_0 = int(model.factor * (psf_w - crop_size_y) // 2) + model.PAD_SIZE1
    pad_y_1 = (
        int(model.factor * (psf_w - crop_size_y) // 2) + model.PAD_SIZE1 + offset_y
    )
    PADDING = (pad_y_0, pad_y_1, pad_x_0, pad_x_1)
    return F.pad(x, PADDING, "constant", 0)
...
def soft_2d_gradient2_rgb(model, v, h, tau):
    ...
    #     3,
    #     1,
    #     model.DIMS1 + 2 * model.PAD_SIZE1,
    #     dtype=torch.float32,
    #     device=model.cuda_device,
    # )
    z2 = torch.zeros_like(h)[:, :, :, :1]

    vv = torch.cat([v, z1], 2)
    hh = torch.cat([h, z2], 3)

    mag = torch.sqrt(vv * vv + hh * hh + 1e-12)
    z0 = torch.tensor(1e-12).to(vv.device)
...
def normalize_image(image):
    ...
    image_max_eye = (
        torch.eye(out_shape[0], dtype=torch.float32, device=image.device)
        * 1
        / image_max
    )
    image_normalized = torch.reshape(
        torch.matmul(image_max_eye, image_flat),
        (out_shape[0], out_shape[1], out_shape[2], out_shape[3]),
    )
...
def make_laplacian(model):
    lapl = np.zeros(
        [model.DIMS0 + 2 * model.PAD_SIZE0, model.DIMS1 + 2 * model.PAD_SIZE1]
    )
    lapl[0, 0] = 4.0
    lapl[0, 1] = -1.0
    lapl[1, 0] = -1.0
